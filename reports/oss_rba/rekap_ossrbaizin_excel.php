...
$sheet->setCellValue('A3', "Tanggal Terbit Izin: ".tgl1($search1)." s/d ".tgl1($search2));
...
$tabel = "SELECT*FROM view_ossrbaizin WHERE id IS NOT NULL";		
...
for($i = 0; $i < mysql_num_fields($query); $i++){
	...
	while ($r= mysql_fetch_array($query)){
		...
		for($y = 0; $y < mysql_num_fields($query); $y++){
			...
			elseif(($name=="day_of_tanggal_terbit_oss") or ($name=="day_of_tgl_izin")) $sheet->setCellValue($col.$j,tgl1($r[$name]));
...
$writer->save('Data Izin.xlsx');
echo "<script>window.location = 'Data Izin.xlsx'</script>";
?>
