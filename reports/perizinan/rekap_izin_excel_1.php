...
$header = array("No.","Nama Pemohon","Telp./HP. Pemohon","Lokasi","Nomor Izin","Tanggal Penetapan","Akhir Masa Berlaku");
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', STRTOUPPER($search3));
...
$objPHPExcel->getActiveSheet()->getStyle('A5:'.$cols.'5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	...
	$query = mysql_query($tabel);
		while ($r= mysql_fetch_array($query)){
			...
			$value = array ($i,$r['nama'],$contact,trim($r['lokasi_izin']),$r['no_izin'],$tgl2,$masaberlaku);
			$col = "A";
			for($y=0;$y< count($header);$y++){
				$cols = $col;
