...
$header = array("No.","Nama Pemohon","Nama Perusahaan","Jenis Izin","Jenis Permohonan","Telp./HP. Pemohon","Jenis Reklame","Isi Reklame","Ukuran Reklame","Lokasi Reklame","Nomor Izin","Tanggal Penetapan","Akhir Masa Berlaku");
	...
	$data = json_decode($konten, true);
		...
		foreach ($data["data"]["data"] as $key=>$r) {
			...
			if($tampil){	
				...
				if($tampil2){	
					$value = array ($i,$r['nama'],$usaha,$r['jenis_izin'],$r['jenis_permohonan'],$contact,$r['jenis_reklame'],$r['isi_reklame'],$r['ukuran'],trim($r['lokasi_pasang']),$r['no_izin'],$tgl2,$masaberlaku);
					$col = "A";
					for($y=0;$y< count($header);$y++){
						$cols = $col;
