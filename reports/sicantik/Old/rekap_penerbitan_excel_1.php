...
$header = array("No.","Nomor Permohonan","Nama Pemohon","Nama Usaha","Telp./HP. Pemohon","Lokasi","Nomor Izin","Tanggal Penetapan","Akhir Masa Berlaku");
...
if($search3)$objPHPExcel->getActiveSheet()->setCellValue('A2', STRTOUPPER($jns_izin['jenis_izin']));
else $objPHPExcel->getActiveSheet()->setCellValue('A2', 'DAFTAR PERMOHONAN PERIZINAN');
$objPHPExcel->getActiveSheet()->setCellValue('A3', 'BULAN : '.STRTOUPPER(bulantahun($search2)));
	...
	$data = json_decode($konten, true);
		...
		foreach ($data["data"]["data"] as $key=>$r) {
			...
			if($tampil){	
				...
				if($tampil2){
					$value = array ($i,$r['no_permohonan'],$r['nama'],$usaha,$contact,trim($r['lokasi_izin']),$r['no_izin'],$tgl2,$masaberlaku);
					$col = "A";
					for($y=0;$y< count($header);$y++){
						$cols = $col;
