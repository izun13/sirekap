...
$header = array("No.","Nama Pemohon","Jenis Izin","Jenis Permohonan","Telp./HP. Pemohon","Lokasi","Status Proses Terakhir");
...
$objPHPExcel->getActiveSheet()->getStyle('A5:'.$cols.'5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	...
	$sumber = "https://ws.sicantik.go.id/api/TemplateData/keluaran/36461.json";
	...
	$data = json_decode($konten, true);
		...
		foreach ($data["data"]["data"] as $key=>$r) {
			...
			if($tampil){	
				...
				if($tampil2){	
					$value = array ($i,$r['nama'],$r['jenis_izin'],$r['jenis_permohonan'],$contact,trim($r['lokasi_izin']),$r['nama_proses']);
					$col = "A";
					for($y=0;$y< count($header);$y++){
						$cols = $col;
