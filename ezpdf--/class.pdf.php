      ...
      foreach($o['info'] as $k=>$v){
        switch($k){
          ...
          case 'openHere':
            ...
            if ($this->fit_to_page) {
            	$fit_code = 'pp.flags &= ~(fv.suppressCenter | fv.suppressRotate) ; ';
            }else{
            	$fit_code = 'pp.flags |= (fv.suppressCenter | fv.suppressRotate) ; ';
            }
            $res.="\n".'/OpenAction '.'<< /Type /Action /S /JavaScript /JS (var pp = getPrintParams(); pp.interactive = pp.constants.interactionLevel.automatic; var fv = pp.constants.flagValues; '.$fit_code;
            $res.='\ this.print\({bUI: false, bSilent: false, bPrintAsImage: true, printParams: pp}\);) >>';
            break;
        }
      }
      $res.=" >>\nendobj";
