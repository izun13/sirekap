...
if ($akses['pokok']){  
   ...
   echo"<li class='has-sub' id='$active'><a href='?send='><span>Data Pokok</span></a>
      <ul>";
		...
		$aktiv = '';
		if(strstr($_GET['send'],"sektor")) $aktiv = 'aktiv'; 
...
if ($akses['nswi']){
   ...
   echo"<li class='has-sub' id='$active'><a href='?send='><span>OSS NSWI</span></a>
      <ul>";
        ...
        echo" <li class=$aktiv><a href='?send=datainvestasi'><span>Realisasi Investasi</span></a></li>";
   echo"</ul>
   </li>";
}
...
if ($akses['simpadu']){  
   $active = '';
   if(strstr($_GET['send'],"simpadu1")) $active = 'active'; 
   if(strstr($_GET['send'],"simpadu2")) $active = 'active'; 
   if(strstr($_GET['send'],"simpadu3")) $active = 'active';
   if(strstr($_GET['send'],"simpadu4")) $active = 'active';
   echo"<li class='has-sub' id='$active'><a href='?send='><span>Rekap Simpadu</span></a>
      <ul>";
		$aktiv = '';
		if(strstr($_GET['send'],"simpadu1")) $aktiv = 'aktiv'; 
        echo" <li class=$aktiv><a href='?send=simpadu1'><span>Rekap Perizinan</span></a></li>";
...
if ($akses['sicantik']){		
   $active = '';
   if(strstr($_GET['send'],"sicantik0")) $active = 'active'; 
   if(strstr($_GET['send'],"sicantik1")) $active = 'active'; 
   if(strstr($_GET['send'],"sicantik2")) $active = 'active';
   if(strstr($_GET['send'],"sicantik3")) $active = 'active';
   if(strstr($_GET['send'],"sicantik4")) $active = 'active';
   if(strstr($_GET['send'],"sicantik5")) $active = 'active';
   echo"<li class='has-sub' id='$active'><a href='?send='><span>Rekap Sicantik</span></a>
      <ul>";
        ...
        echo" <li class=$aktiv><a href='?send=sicantik1'><span>Rekap Penerbitan Izin</span></a></li>";
