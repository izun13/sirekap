...
?>
<div class="judul">Rekap Perizinan Reklame Sicantik</div>

<form action='<?php echo "?send=".$to_page;?>' method='post' autocomplete="off"> 
<table>
	<tr><td width="200">Periode Tanggal </td><td>:</td>
		<td width=""> <select name="search0">
		<?php 
			$periode=array("Pengajuan","Pengesahan");
			for ($x=0;$x<2;$x++){
				$selected = "";
	...
	<tr><td>Jenis Izin  </td><td>:</td>
		<td width="400"> <select name="search3" id="pilih">
			...
			<?php
			$query_jns=mysql_query("SELECT*FROM jenis_izin WHERE jenis_izin LIKE '%$search7%'order by jenis_izin asc");
	...
	<tr><td width="200">No. Permohonan</td><td>:</td>
...
<?php	
if(($act == "Tampilkan") or (($search1 != "") and ($search2 != ""))){
	
	if(($search1 == "") or ($search2 == "")){
		?>
			<script language="JavaScript">alert('Periode Tanggal Pengajuan/Pengesahan Harus Diisi !');
	...
	if($search6 != "") $tabel .= " AND no_permohonan LIKE '%$search6%'";
	...
	echo"<a href='reports/sicantik/rekap_reklame_excel.php?$searchlink' target='_blank'><img src=\"img/excel.png\" width='50' title='cetak excel'></img></a>";
	echo"&nbsp; <a href='reports/sicantik/rekap_reklame_pdf.php?$searchlink'  target='_blank'><img src=\"img/pdf.png\" width='38' title='cetak PDF'></img></a>";
	//echo"&nbsp; <a href='reports/sicantik/rekap_izin_graph_4.php?$searchlink'  target='_blank'><img src=\"img/graph1.png\" width='38' title='Lihat Grafik'></img></a>";
?>
	<table class="tabelbox1" width="800">
		<tr>
			...
			<th><b>Rincian Teknis Reklame</b></th>	
		</tr>
		<?php
		$starting = starting();
		$recpage = recpage();
		...
		while ($r= mysql_fetch_array($query)){
			...
			if($tampil == 1){
				...
				echo "<td align='left'>$r[nama_perusahaan]</td>";
				...
				echo "<td align='center'><a href=?send=detil-sicantik4/$id/$starting/$search'><img src='img/draf.png' width='25' title='Rincian'></img></a></td>";
