...
<div><span class='judul'>Data OSS RBA NIB</span></div>

<form action='<?php echo "?send=".$to_page;?>' method='post' autocomplete="off" id="formulir"> 
<table>
<?php
$tabel = "SELECT id,nib,day_of_tanggal_terbit_oss,nama_perusahaan,status_penanaman_modal,uraian_jenis_perusahaan,alamat_perusahaan,kab_kota,email,nomor_telp FROM oss_rba_nibs WHERE id IS NOT NULL";	
//,status_perusahaan
Function title($name){
	if($name == "id")$title = "ID";
	...
	if($name == "nomor_telp")$title = "No. Telpon";
	//if($name == "status_perusahaan")$title = "Status Perusahaan";
	
	return $title;
}	

echo "<tr><td width='150px'>Tanggal Terbit NIB</td><td>:</td><td width='250px'><input type='text' size='10' name='search1' value='$search1' id='inputField1' class='search'>";
...
<?php
	...
	echo"<td align='right'><a href='reports/oss_rba/rekap_ossrbanib_excel.php?search=$search' target='_blank'><img src=\"img/excel.png\" width='50' title='cetak excel'></img></a>";
	echo"&nbsp; <a href='reports/oss_rba/rekap_nib_json.php?send=$search1;$search2' target='_blank'><img src=\"img/json.png\" width='38' title='Cetak Json'></img></a></td>";
?>
</tr>
</table>
</form>
