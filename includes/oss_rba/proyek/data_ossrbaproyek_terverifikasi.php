...
<div><span class='judul'>Data OSS RBA Proyek Terverifikasi</span></div>
...
//"bangunan_gedung","modal_kerja","lain_lain","tki",verifikasi,tgl_verifikasi, uraian_jenis_perizinan,sektor,
$tabel = "SELECT id,nama_perusahaan,nib,uraian_status_penanaman_modal,uraian_jenis_perusahaan,uraian_risiko_proyek,uraian_skala_usaha,
		kbli,judul_kbli,jumlah_investasi,tambah_investasi,tki,status_perusahaan,status_kbli,tgl_verifikasi FROM view_proyek WHERE id IS NOT NULL";	//day_of_tgl_izin,uraian_jenis_perizinan,
...
Function title($name){
	...
	if($name == "tambah_investasi")$title = "Jumlah Investasi Terverifikasi";
...
echo "<tr><td width='150px'>Tanggal Verifikasi Proyek</td><td>:</td><td width='250px'><input type='text' size='10' name='search1' value='$search1' id='inputField1' class='search'>";
...
echo "</select>";*/
//echo "<tr><td width='150px'>Bulan</td><td>:</td><td width='250px'><input type='text' size='10' name='search1' value='$search1' id='' class='search'>";
...
if(($search1 != "") and ($search2 != "")){
	$tabel .= " AND tgl_verifikasi >= '$search1' AND tgl_verifikasi <= '$search2'";
...
<table width="100%">
<tr><td>Group by NIB Perusahaan : <input type='checkbox' name='group' onchange="tampilkan()" <?php echo $checked;?>>
<?php
/*$checked = "";
if($group == 1)$checked = "checked";
echo "<input type='radio' name='group' value='1' onchange='tampilkan()' $checked> Nama Perusahaan ";
$checked = "";
if($group == 2)$checked = "checked";
echo "<input type='radio' name='group' value='2' onchange='tampilkan()' $checked> Alamat Perusahaan ";
$checked = "";
if($group == "")$checked = "checked";
echo "<input type='radio' name='group' value='' onchange='tampilkan()' $checked> Semua ";*/

	echo"<a href='reports/oss_rba/rekap_ossrbaproyek_verified_excel.php?search=$search&act=$group' target='_blank'><img src=\"img/excel.png\" width='50' title='cetak excel'></img></a>";
...
* Klik judul kolom untuk mengurutkan.-->
	<table class="tabelbox1">
		...
		//if($group == 1) $tabel .= " GROUP BY nama_perusahaan";
		//if($group == 2) $tabel .= " GROUP BY alamat_usaha";
		if($group) $tabel .= " GROUP BY nib";
