...
if($_POST["tombol"]=="Import"){
	/*if(($search1 == "") or ($search2 == "")){
		...
		//$field = array ("id_proyek","nib","npwp_perusahaan","nama_perusahaan","uraian_status_penanaman_modal","uraian_jenis_perusahaan","uraian_risiko_proyek",
		//"uraian_skala_usaha","alamat_usaha","kecamatan_usaha","kelurahan_usaha","longitude","latitude","kbli","judul_kbli","kl_sektor_pembina","nama_user","nomor_identitas_user","email","nomor_telp",
		//"mesin_peralatan","mesin_peralatan_impor","pembelian_pematangan_tanah","bangunan_gedung","modal_kerja","lain_lain","jumlah_investasi","tki");
		
		$field = array ("id_proyek","nib","npwp_perusahaan","nama_perusahaan","uraian_status_penanaman_modal","uraian_jenis_perusahaan","uraian_risiko_proyek","nama_proyek",
		"uraian_skala_usaha","alamat_usaha","kecamatan_usaha","kelurahan_usaha","longitude","latitude","kbli","judul_kbli","kl_sektor_pembina","nama_user","nomor_identitas_user","email","nomor_telp",
		"luas_tanah","satuan_tanah","mesin_peralatan","mesin_peralatan_impor","pembelian_pematangan_tanah","bangunan_gedung","modal_kerja","lain_lain","jumlah_investasi","tki");
		...
		//$reset_autoincrement = mysql_query("ALTER TABLE oss_rba_proyek_ori AUTO_INCREMENT = 1482");
		...
		foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
			...
			$highestColumn = 'AF'; //$worksheet->getHighestColumn(); // e.g 'F'
			...
			for ($row = 2; $row <= $highestRow; ++ $row) {
				...
				//$kolom .= "tgl_input";
				//$isi .= "'".$date."'";
				//$update .= ",tgl_input='".$date."'";
				//echo "$isi<br>";
				
				$cek=mysql_num_rows(mysql_query("select*from oss_rba_proyek_ori where id_proyek='$id'"));
				if($cek == 0){
					...
					$tambah="insert into oss_rba_proyek_ori ($kolom) values ($isi)";
					$hasil=mysql_query($tambah);
					if (!$hasil) echo "Input Gagal :".mysql_error().$kolom."--".$isi."<br>";
				}/*else{
					$r_proyek=mysql_fetch_array(mysql_query("select*from oss_rba_proyek_ori where id_proyek='$id'"));
					$ubah="update oss_rba_proyek_ori set $update where id='$r_proyek[id]'";
...
<div><span class='judul'>Import Data OSS RBA Proyek All (https://oss.go.id/)</span></div>
...
<?php
if($exist){
	echo "<td>File Terupload </td><td><input type='text' name='openfile' value='$nama_file' class='readonly' readonly></td>";
	echo "<td>Bulan : <select name='search1' id=''>";
	...
	echo "</select>";
	echo " Tahun : <input type='text' size='10' name='search2' value='$search2' id='' class='search'>";*/
	echo "<input type='submit' name='tombol' value='Import'></td>";
}
?>
</tr>	
</table>
