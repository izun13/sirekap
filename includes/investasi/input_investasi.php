...
<body>
<?php
$tabel = "SELECT*FROM data_investasi WHERE id='$id'";					
...
if($_POST["submit"]=="SIMPAN"){
	...
	$cek=mysql_num_rows(mysql_query("select*from data_investasi where id='$id'"));
	if($cek == 0){
		$tambah="insert into data_investasi ($kolom) values ($isi)";
		$hasil=mysql_query($tambah);
		if (!$hasil) echo "Input Gagal :".mysql_error()."<br>";
	}else{
		$ubah="update data_investasi set $update where id='$id'";
	...
	if ($hasil){
		...
		<script language="JavaScript">alert('data realisasi investasi telah diupdate !');
			document.location.href='?send=datarinvestasi//<?php echo $starting; ?>/<?php echo $search; ?>';
		</script>
		<?php
	}
}
...
<div><span class='judul'>Input Data Investasi</span></div>
