...
<div><span class='judul'>Data NON IUMK OSS NSWI</span></div>
...
for ($j=1;$j<=$filters;$j++){
	...
	echo "<td width='230px'><select name='$kolom' id='$pilih'><option value=''>Pilih Kolom</option>";
		$tabel = mysql_query("SELECT nama_perusahaan,nib,kbli,nama_izin,tanggal_izin,uraian,status_izin,investasi FROM oss_noniumk");	
...
?>
<tr>
<td colspan="5" align="right"><input type='submit' name='tombol' value='Tampilkan'></td></tr>
</table>
...
<tr><td>
<?php
	echo"<a href='reports/oss/rekap_ossnoniumk_excel.php?search=$search&group=$group'><img src=\"img/excel.png\" width='50' title='cetak excel'></img></a>";
	echo"&nbsp; <a href='reports/oss/rekap_ossnoniumk_pdf.php?search=$search&group=$group'  target='_blank'><img src=\"img/pdf.png\" width='38' title='cetak PDF'></img></a>";
?>
</td></tr>
</table>
...
* Klik judul kolom untuk mengurutkan.-->
	<table class="tabelbox1">
		...
		$tabel2 = "SELECT nama_perusahaan,nib,tanggal_nib,status_pm,kbli,nama_izin,tanggal_izin,uraian,status_izin,investasi,tenaga_kerja FROM oss_noniumk WHERE id IS NOT NULL";	
		...
		if($group) $tabel2 .= " GROUP BY nama_perusahaan";
		
		//if(strpos($act,"asc")) {$kolom_order = preg_replace("/asc/","",$act); $tabel2 .= " ORDER BY $kolom_order asc";}
		//if(strpos($act,"desc")) {$kolom_order = preg_replace("/desc/","",$act); $tabel2 .= " ORDER BY $kolom_order desc";}
		...
		while ($r= mysql_fetch_array($query)){
			...
			for($i = 0; $i < mysql_num_fields($query); $i++){
				$name = mysql_field_name($query, $i);
				if($name == "investasi") echo "<td align='right'>".rupiah($r[$name])."</td>";
				elseif(($name == "status_nib") or ($name == "tenaga_kerja")) echo "<td align='center'>$r[$name]</td>";
